...
/**
 * return the value from Some if not None, else return [defaultValue]
 *
 * @return [T] the value from Some or [defaultValue]
 */
fun <T : Any> Option<T>.getOrElse(defaultValue: T): T =
  when (this) {
...
/**
 * return Some if it is Some, else return [default]
 *
 * @return Option<T> return Some if it is Some, or return [default]
 */
fun <T : Any> Option<T>.orElse(default: Option<T>) =
  when (this) {
    is Option.Some -> Option.Some(value = value)
 ...
 * @return Option<S>. return Option<S> if it is Some, else return None
 */
fun <T : Any, S : Any> Option<T>.map(transform: (T) -> S): Option<S> =
  when (this) {
 ...
 * @return Option<S>. return Option<S> if it is Some, else return None
 */
fun <T : Any, S : Any> Option<T>.flatMap(transform: (T) -> Option<S>): Option<S> =
  when (this) {
